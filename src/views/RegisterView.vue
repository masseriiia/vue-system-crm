    ...
    <form class="flex flex-col gap-4" @submit.prevent="handleRegisterSubmit">
      <div>
        <label>
          ...
          <AppInput type="email" v-model="register.email" :error="emailErrorText" placeholder="Введите почту"/>
        </label>
      </div>
      <div>
        <label>
          <span class="label-text">Пароль</span>
          <AppInput type="password" v-model="register.password" :error="passwordErrorText" placeholder="Введите пароль"/>
        </label>
      </div>
